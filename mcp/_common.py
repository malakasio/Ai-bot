...
def require_str(
    value: Any, name: str, *, max_len: int = 4096, pattern: Optional[str] = None
) -> str:
    require(isinstance(value, str), f"{name!r} must be a string")
    require(0 < len(value) <= max_len, f"{name!r} must be 1..{max_len} chars (got {len(value)})")
    if pattern is not None:
        require(re.fullmatch(pattern, value) is not None, f"{name!r} does not match {pattern!r}")
    return value
...
def require_int(
    ...
    require(isinstance(value, int) and not isinstance(value, bool), f"{name!r} must be an int")
    if lo is not None:
...
def require_in(value: Any, name: str, choices: list[Any]) -> Any:
    require(value in choices, f"{name!r} must be one of {choices!r}; got {value!r}")
    return value
...
class Server:
    ...
    def tool(
        self, name: str, description: str = "", input_schema: Optional[dict[str, Any]] = None
    ) -> Callable[[Handler], Handler]:
        """Decorator: register an async handler as a tool."""

        def deco(fn: Handler) -> Handler:
            require(inspect.iscoroutinefunction(fn), f"tool {name!r} handler must be async")
            self.tools[name] = Tool(
                name=name,
                description=description
                or (fn.__doc__ or "").strip().splitlines()[0:1]
                and (fn.__doc__ or "").strip().splitlines()[0]
                or "",
                handler=fn,
    ...
    async def dispatch(self, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return err(str(e), code="invalid_input", server=self.name, tool=tool_name)
        except Exception as e:
            return err(repr(e), code="exception", server=self.name, tool=tool_name)
