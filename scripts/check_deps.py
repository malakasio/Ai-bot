...
def collect() -> list[Dep]:
    deps: list[Dep] = [
        ...
        Dep("uvicorn", "python", "voice", required=False, pip="uvicorn", note="ASGI runner"),
        Dep(
            "websockets",
            "python",
            "voice",
        ...
        # Daemons
        Dep("systemctl", "bin", "kairos/rollback", required=False, note="service restart hook"),
        # Credentials (env)
        Dep(
            "ANTHROPIC_API_KEY",
            "env",
...
def render_table(deps: list[Dep], capability_filter: Optional[str] = None) -> str:
    rows = [d for d in deps if not capability_filter or d.capability == capability_filter]
    ...
    out = []
    out.append(f"{'NAME':<22} {'KIND':<7} {'STATUS':<10} {'REQ':<4} DETAIL")
    out.append("-" * 78)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        "--strict", action="store_true", help="exit 1 if any required dep is missing"
    )
    parser.add_argument("--json", action="store_true", help="emit JSON instead of the human table")
    parser.add_argument("--capability", default=None, help="filter table to one capability group")
