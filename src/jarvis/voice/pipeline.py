...
class VoicePipeline:
    ...
    async def process_audio_input(
        ...
        # collected_tokens is set by _llm_tts_stream before it starts yielding.
        # We read it AFTER the generator finishes (or on barge-in interruption).
        self.session.collected_tokens = ""
        ...
        try:
            async for audio_chunk in self._llm_tts_stream(user_text):
                if await self.vad.wait_for_interrupt(timeout=0.01):
                    ...
                    # Use whatever was collected before the interrupt
                    await self._handle_barge_in(self.session.collected_tokens)
                    return
        ...
        # Append assistant response to history (now correctly populated)
        if self.session.collected_tokens:
            self.session.history.append({
                "role": "assistant",
                "content": self.session.collected_tokens,
            })
    ...
    async def _llm_tts_stream(self, user_text: str) -> AsyncIterator[bytes]:
        ...
        NOTE: stores full_response on self.session.collected_tokens so the
        caller can append it to history after the generator is exhausted.
        `return value` in an async generator is silently discarded by the
        caller's `async for` loop — never use it to pass data back.
        """
        ...
        # Store on session so process_audio_input can append to history.
        # Cannot return from an async generator — value is discarded.
        self.session.collected_tokens = full_response
