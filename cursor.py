...
def run_agent():
    ...
    all_files = [
        f for f in os.listdir(".") if os.path.isfile(f) and f.endswith((".py", ".txt", ".json"))
    ]
    files_list = "\n".join(all_files)
    ...
    try:
        ...
        if target_file not in all_files:
            print(
                f"⚠️ Ο {MODEL} πρότεινε το '{target_file}', αλλά δεν το βρίσκω. Δοκιμάζω το main.py."
            )
            target_file = "main.py"
        ...
        edit_data = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": f"Άλλαξε αυτόν τον κώδικα σύμφωνα με την οδηγία: {instruction}\n\nΚώδικας:\n{old_code}",
                }
            ],
        }
