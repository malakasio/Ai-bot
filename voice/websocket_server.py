...
class ConnectionManager:
    ...
    def __init__(self, *, max_connections: int = 64, idle_timeout_s: float = 300.0) -> None:
        self.max_connections = max_connections
        self.idle_timeout_s = idle_timeout_s
        self._conns: dict[str, Connection] = {}
        self._lock = asyncio.Lock()

    async def register(self, ws: Any, user_id: Optional[str] = None) -> Connection:
        ...
        _logger().info("voice.ws.connect", extra={"conn_id": cid, "active": len(self._conns)})
        return conn
...
async def _audio_iterator(conn: Connection, stop_event: asyncio.Event) -> AsyncIterator[bytes]:
    """Yield audio frames until the client signals stop or the queue closes."""
    while not stop_event.is_set():
        try:
            chunk = await asyncio.wait_for(conn.audio_in.get(), timeout=10.0)
...
def create_app(*, system_prompt: Optional[str] = None) -> Any:
    ...
    async def voice_endpoint(ws: WebSocket) -> None:
        ...
        try:
            ...
            log.error("voice.ws.pipeline_init_failed", extra={"conn_id": conn.id, "exc": repr(e)})
            with suppress(Exception):
        ...
        async def _receiver() -> None:
            ...
            while not conn.closed:
                try:
                    ...
                    log.warning(
                        "voice.ws.receive_failed", extra={"conn_id": conn.id, "exc": repr(e)}
                    )
                    stop_event.set()
                    await conn.audio_in.put(None)
                    return
        ...
        async def _turn_loop() -> None:
            while not stop_event.is_set():
                try:
                    ...
                    log.warning("voice.ws.turn_failed", extra={"conn_id": conn.id, "exc": repr(e)})
                    with suppress(Exception):
                        await ws.send_text(json.dumps({"type": "error", "error": repr(e)}))
                    return
...
def main() -> None:  # pragma: no cover
    ...
    parser.add_argument("--port", type=int, default=int(os.environ.get("VOICE_WS_PORT", "8080")))
    parser.add_argument("--system", default=None, help="optional system prompt for the LLM turn")
