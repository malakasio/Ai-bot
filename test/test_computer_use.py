...
async def test_browser_navigate():
    ...
    result = await server.dispatch(
        "browser_navigate", {"url": "https://example.com", "wait_for": "load"}
    )
...
async def test_browser_screenshot():
    ...
    result = await server.dispatch("browser_screenshot", {"full_page": False})
    ...
    assert result["data"]["size_bytes"] > 1000, (
        f"Screenshot too small: {result['data']['size_bytes']} bytes"
    )
...
async def test_browser_click():
    ...
    await server.dispatch("browser_navigate", {"url": "https://example.com", "wait_for": "load"})

    # Try to click the "More information..." link
    result = await server.dispatch("browser_click", {"selector": "a", "button": "left"})
...
async def test_browser_type():
    ...
    await server.dispatch(
        "browser_navigate", {"url": "https://httpbin.org/forms/post", "wait_for": "load"}
    )

    # Type into the first input field
    result = await server.dispatch(
        "browser_type",
        {"selector": "input[name='custname']", "text": "JARVIS Test", "delay_ms": 10},
    )
    ...
    assert result["data"]["text_length"] == 11, (
        f"Text length mismatch: {result['data']['text_length']}"
    )
...
async def test_security_blocked_url():
    ...
    result = await server.dispatch(
        "browser_navigate", {"url": "https://evil.com", "wait_for": "load"}
    )

    print(f"Result: {result}")

    assert result["ok"] is False, "Expected blocked URL to fail"
    assert "not in JARVIS_NET_ALLOWLIST" in result["error"]["message"], (
        f"Wrong error: {result['error']['message']}"
    )
...
async def test_security_blocked_selector():
    ...
    result = await server.dispatch(
        "browser_click", {"selector": "javascript:alert(1)", "button": "left"}
    )

    print(f"Result: {result}")

    assert result["ok"] is False, "Expected blocked selector to fail"
    assert "blocked pattern" in result["error"]["message"], (
        f"Wrong error: {result['error']['message']}"
    )
